...
{
  usbj_print("[SYSTIMER V40 n="); usbj_print_u32(cp32_timer_irq_ticks);
  usbj_print(phase);
...
CP32_IRAM_EXT PRIVATE void cp32_systimer_report_return(void)
{
  ...
  usbj_print("[SYSTIMER V40 return deadline=");
...
CP32_IRAM_EXT PRIVATE void cp32_trace_fs_return(void)
{
  ...
  reports++;
  usbj_print("[CTX V40 FS-RETURN n="); usbj_print_u32(reports);
...
CP32_IRAM_EXT PRIVATE void cp32_trace_clock_receive(void)
{
  ...
  saved_ps = lock_save();
  usbj_print("[CLOCK V40 received="); usbj_print_u32(received);
...
PUBLIC void systimer_irq_start()
{
  ...
  usbj_print("[BOOT SYSTIMER V40 conf=");
