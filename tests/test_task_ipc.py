...
enum { OK=0, EINVAL=-1, EFAULT=-2, ELOCKED=-3, E_BAD_DEST=-4,
       ...
       TTY_PROC_NR=-9, IDLE=-7, HARDWARE=-1, FS_PROC_NR=1, LOW_USER=2,
...
static int interrupt_message(struct proc *p, message *m) { (void)p; (void)m; return EFAULT; }
...
static void reset(void) {
  ...
  int active[]={IDLE,TTY_PROC_NR,FS_PROC_NR};
...
int main(void) {
  for (int i=0;i<100;i++) exchange(i&1);
  reset();
  struct proc *fs=proc_addr(FS_PROC_NR),*tty=proc_addr(TTY_PROC_NR);
  ...
  puts("production IPC: 100 BOTH exchanges, SEND wake, owner/frame/queue checks and errors passed");
  return 0;
}
'''
